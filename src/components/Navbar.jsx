...
const Navbar = () => {
  ...
  return (
    <header className="z-[999] relative">
      ...
      ></motion.div>
      <nav
        className="flex fixed w-[20rem] top-[0.15rem] left-1/2 h-12 -translate-x-1/2 py-2 sm:top-[1.7rem] sm:h-[initial]  sm:w-[22rem] sm:py-0 px-3"
        ...
        <ul className="flex  flex-wrap items-center justify-center gap-2 text-[1rem] font-medium text-gray-300 sm:w-[initial] sm:flex-nowrap sm:gap-5">
          {links.map((link) => (
            ...
            >
              <Link
                className={clsx(
                  "flex w-full items-center justify-center px-3 py-3 hover:text-teal-500 transition cursor-pointer",
                  { "text-teal-500": activeSection === link.name }
                )}
                href={link.hash} //!extra
                activeClass="active"
                to={link.name.toLowerCase()} //might have to change
                spy={true}
                smooth={true}
                hashSpy={true}
                offset={10}
